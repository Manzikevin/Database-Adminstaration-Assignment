...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($first_name) && !empty($last_name) && !empty($email)) {
        try {
            ...
            $feedback = ['status' => 'success', 'message' => 'Faculty member added successfully!'];
...
$instructors = $pdo->query("SELECT i.*, d.dept_name FROM instructors i LEFT JOIN departments d ON i.dept_id = d.dept_id ORDER BY i.instructor_id DESC")->fetchAll();
?>
<!DOCTYPE html>
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6 border-b border-slate-200/80">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Manage academic staff, department assignments, and contact details.</p>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200/90 overflow-hidden">
                <div class="p-6 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Active instructors and faculty assignments in database.</p>
                    </div>
